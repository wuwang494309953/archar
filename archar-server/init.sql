...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT =''菜单栏表'';

-- ----------------------------
-- Table structure for sys_query_info
-- ----------------------------
DROP TABLE IF EXISTS `sys_query_info`;
CREATE TABLE `sys_query_info`
(
    `id`          bigint(20)   NOT NULL,
    `query_code`  varchar(50)  NOT NULL COMMENT '' 查询code '',
    `query_name`  varchar(100) NOT NULL COMMENT '' 查询名字 '',
    `description` varchar(255) DEFAULT NULL COMMENT '' 注释说明 '',
    `create_time` datetime     NOT NULL COMMENT '' 创建时间 '',
    `update_time` datetime     NOT NULL COMMENT '' 更新时间 '',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT =''查询数据'';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT =''查询数据的字段信息（前端列表展示和查询用）'';
